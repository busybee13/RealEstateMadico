...
public class UpdatePropertiesActivity extends ActionBarParentActivity implements
		SelectAmenitiesComunicator {
	Cursor cursor;
	String[] categories_name, county_name, city_name, purpose_name,
			time_rate_name, status_name;
	int[] categories_id, county_id, city_id;
	String[] purpose_id, time_rate_id, status_id;
	String lat = "", lng = "";
	Spinner typeSpinner, countySpinner, purposeSpinner, citiesSpinner,
			markerSpinner, timeRateSpinner, statusSpinner;
			...
			if (data.containsKey(KEY_MARKER_RESPONSE)) {
				...
				try {
					...
					for (int i = 0; i < jsonArray.length(); i++) {
						...
						String path = countyJSON.getString("path");
					...
					new JSONFetchTask(getResources().getString(
							...
							+ "estates?estate_id=" + properties_id,
							handler_estates, KEY_ESTATES_REPONSE).execute();
	...
	Handler handler_estates = new Handler() {
		public void handleMessage(android.os.Message msg) {
			Bundle data = msg.getData();
			if (data.containsKey(KEY_ESTATES_REPONSE)) {
				String jsonString = data.getString(KEY_ESTATES_REPONSE);
				try {
					JSONArray jsonArray = new JSONArray(jsonString);
	...
	public void parseProperties(JSONObject jsonObj) {
		try {
			...
			String contentText = jsonObj.getString(Properties.TAG_CONTENT);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.update_properties_activity_layout);
		...
		status_id = getResources().getStringArray(R.array.status_id);
		status_name = getResources().getStringArray(R.array.status_name);
		...
		ArrayAdapter<String> statusAdapter = new ArrayAdapter<String>(this,
				android.R.layout.simple_list_item_1, status_name);
		...
		statusSpinner
				.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
					...
					public void onItemSelected(AdapterView<?> arg0, View arg1,
							int arg2, long arg3) {
						// TODO Auto-generated method stub
						status_selected = Integer.valueOf(status_id[arg2]);
					}
		...
		btnUpload.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (!Ultils
						.isConnectingToInternet(UpdatePropertiesActivity.this)) {
					...
					public void run() {
						...
						try {
							...
							reqEntity.addPart("bedrooms", new StringBody(
									bedroomText));
							reqEntity.addPart("status", new StringBody(
									status_selected + ""));
